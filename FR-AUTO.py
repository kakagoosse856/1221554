...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "FR-AUTO.m3u8")

servers = {}

print("[INFO] استخراج قنوات FR وترتيبها حسب السيرفر فقط .m3u8...")
...
for idx, src in enumerate(SOURCES, start=1):
    ...
    for i, line in enumerate(lines):
        ...
        if "max" in channel_name.lower() or "max" in url.lower():
            channel_name = "FR HD"
        elif "4k" in channel_name.lower() or "HD" in url.lower():
            channel_name = "FR 4K"
    ...
    for server, channels in servers.items():
        for ch in sorted(channels.keys()):
            for url in sorted(channels[ch]):
                f.write(
                    f'#EXTINF:-1 group-title="✪ FR AUTO | {server}",{ch}\n'
                )
                f.write(url + "\n")
